...
func (h *AuthHandler) Register(w http.ResponseWriter, r *http.Request) {
	...
	data, token, err := h.authService.Register(req)
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
	...
	registerResponse.UserentityToRegisterResponse(data, token)

	rabbitmqResponse, err := rabbitmq.MappingJsonToRabbitMQMessage(registerResponse)
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(baseResponse.ErrorResponse(err.Error()))
		return
	...
	go rabbitmq.PublishMessage((*rabbitmq.RabbitMQConnection)(&h.config.RabbitMQConn), rabbitmqResponse, "customer_register")

	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(baseResponse.SuccessResponse(constant.REGISTER_SUCCESS, registerResponse))
}
...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	go rabbitmq.PublishMessage((*rabbitmq.RabbitMQConnection)(&h.config.RabbitMQConn), rabbitmqResponse, "customer_login")
